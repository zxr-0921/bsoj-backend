...
public class PostThumbServiceImpl extends ServiceImpl<PostThumbMapper, PostThumb>
    ...
    @Override
    public List<Long> queryMyThumb(Long id) {
        //1. 根据用户id查询点赞表，获取所有的点赞记录
        QueryWrapper<PostThumb> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("userId", id);
        List<PostThumb> postThumbList = this.list(queryWrapper);
        //2. 获取点赞记录中的帖子id
        if (!postThumbList.isEmpty()) {
            return postThumbList.stream().map(PostThumb::getPostId).collect(Collectors.toList());
        }
